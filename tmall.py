# -*- coding: utf-8 -*-
# @Author: liang
# @Date:   2017-11-30 20:17:47
# @Last Modified by:   liang
# @Last Modified time: 2017-11-30 20:48:14
import requests
import json
import csv
import random
...
class TM_producs(object):
	...
	def get_file(self):
		...
		with open(self.filename,'w', encoding='utf_8_sig') as f:
	...
	def get_products(self,page):
		...
		title = ['item_id', 'price', 'quantity', 'sold', 'title', 'totalSoldQuantity', 'url', 'img']
		with codecs.open(self.filename, 'a', encoding='utf_8_sig') as f:
			# f.write(codecs.BOM_UTF32)
			writer = csv.DictWriter(f, fieldnames=title)
			writer.writerows(products)
		# print (len(products))
			# for p in products:
			# 	writer.writerow({'item_id': p.get('item_id'), \
			# 		'price': p.get('price'), \
			# 		'quantity': p.get('quantity'),\
			# 		'sold': p.get('sold'), \
			# 		'title': p.get('title', '').encode('utf-8'), \
			# 		'totalSoldQuantity': p.get('totalSoldQuantity'), \
			# 		'url': p.get('url'), \
			# 		'img': p.get('img')})
